...
struct YahooFinanceAPIClient {
    ...
    static func fetchQuotes(symbols: [String]) async throws -> YahooFinanceQuoteResponse {
        guard var quoteURLComponents = YahooFinanceAPIClient.URLs.quote else {
            throw FetchError.badURLComponents
        }
        quoteURLComponents.queryItems = [
            URLQueryItem(name: "symbols", value: symbols.joined(separator: ","))
        ]
        guard let quoteURL = quoteURLComponents.url else {
